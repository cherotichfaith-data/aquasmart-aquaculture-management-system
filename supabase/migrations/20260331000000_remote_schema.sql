...
begin
  ...
  cur_biom as (
    select
      s.system_id,
      (
        select d.biomass_last_sampling
        from public.daily_fish_inventory_table d
        where d.system_id = s.system_id and d.inventory_date = v_start
      ) as b0,
      (
        select d.biomass_last_sampling
        from public.daily_fish_inventory_table d
        where d.system_id = s.system_id and d.inventory_date = v_end
      ) as b1
    from sys s
  ),
  prev_biom as (
    select
      ...
      (
        select d.biomass_last_sampling
        from public.daily_fish_inventory_table d
        where d.system_id = s.system_id and d.inventory_date = v_prev_start
      ) as b0,
      (
        select d.biomass_last_sampling
        from public.daily_fish_inventory_table d
        where d.system_id = s.system_id and d.inventory_date = v_prev_end
      ) as b1
    from sys s
  ),
  cur_w as (
    select
      ...
      coalesce((
        ...
        where h.system_id = s.system_id and h.date > v_start and h.date <= v_end
      ), 0) as harvest_kg,
      coalesce((
        select sum(fs.total_weight_stocking)
        from public.fish_stocking fs
        where fs.system_id = s.system_id and fs.date > v_start and fs.date <= v_end
      ), 0) as stocked_kg,
      coalesce((
        select sum(ft.total_weight_transfer)
        from public.fish_transfer ft
        where ft.origin_system_id = s.system_id
          and ft.date > v_start
          and ft.date <= v_end
      ...
      coalesce((
        select sum(ft.total_weight_transfer)
        from public.fish_transfer ft
        where ft.target_system_id = s.system_id
          and ft.date > v_start
          and ft.date <= v_end
          and public.transfer_impacts_efcr(ft.transfer_type, ft.origin_system_id, ft.target_system_id)
      ), 0) as tin_kg
  ...
  prev_w as (
    select
      ...
      coalesce((
        select sum(h.total_weight_harvest)
        from public.fish_harvest h
        where h.system_id = s.system_id and h.date > v_prev_start and h.date <= v_prev_end
      ), 0) as harvest_kg,
      coalesce((
        select sum(fs.total_weight_stocking)
        from public.fish_stocking fs
        where fs.system_id = s.system_id and fs.date > v_prev_start and fs.date <= v_prev_end
      ), 0) as stocked_kg,
      coalesce((
        select sum(ft.total_weight_transfer)
        from public.fish_transfer ft
        where ft.origin_system_id = s.system_id
          and ft.date > v_prev_start
          and ft.date <= v_prev_end
          and public.transfer_impacts_efcr(ft.transfer_type, ft.origin_system_id, ft.target_system_id)
      ), 0) as tout_kg,
      coalesce((
        select sum(ft.total_weight_transfer)

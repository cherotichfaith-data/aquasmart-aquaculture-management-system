...
CREATE OR REPLACE FUNCTION "public"."api_daily_fish_inventory"("p_farm_id" "uuid", "p_system_id" bigint DEFAULT NULL::bigint, "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date", "p_cursor_date" "date" DEFAULT NULL::"date", "p_order_asc" boolean DEFAULT false, "p_limit" integer DEFAULT 5000) RETURNS TABLE("inventory_date" "date", "system_id" bigint, "farm_id" "uuid", "number_of_fish" double precision, "number_of_fish_stocked" double precision, "number_of_fish_transferred_in" double precision, "number_of_fish_mortality_aggregated" double precision, "number_of_fish_mortality" double precision, "number_of_fish_transferred_out" double precision, "number_of_fish_harvested" double precision, "feeding_amount" double precision, "feeding_amount_aggregated" double precision, "last_sampling_date" "date", "abw_last_sampling" double precision, "biomass_last_sampling" double precision, "feeding_rate" double precision, "system_volume" double precision, "biomass_density" double precision, "mortality_rate" double precision, "system_name" "text")
    ...
    AS $$
  select *
  from public.api_daily_fish_inventory_rpc(
    p_farm_id => p_farm_id,
    p_system_id => p_system_id,
    p_start_date => p_start_date,
    p_end_date => p_end_date,
    p_cursor_date => p_cursor_date,
    p_cursor_system_id => null,
    p_order_asc => p_order_asc,
    p_limit => p_limit
  );
$$;
...
CREATE OR REPLACE FUNCTION "public"."api_dashboard"("p_farm_id" "uuid", "p_system_id" bigint DEFAULT NULL::bigint, "p_growth_stage" "text" DEFAULT NULL::"text", "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date", "p_time_period" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT NULL::integer, "p_order_desc" boolean DEFAULT true) RETURNS TABLE("system_id" bigint, "farm_id" "uuid", "system_name" "text", "growth_stage" "text", "input_start_date" "date", "input_end_date" "date", "time_period" "text", "sampling_start_date" "date", "sampling_end_date" "date", "total_feed_period" double precision, "efcr_denominator_period" double precision, "efcr" double precision, "efcr_latest_date" "date", "efcr_arrow" "text", "abw" double precision, "abw_latest_date" "date", "abw_arrow" "text", "feeding_rate" double precision, "feeding_rate_latest_date" "date", "feeding_rate_arrow" "text", "mortality_rate" double precision, "mortality_rate_latest_date" "date", "mortality_rate_arrow" "text", "biomass_density" double precision, "biomass_density_arrow" "text", "average_biomass" double precision, "average_number_of_fish" double precision, "feeding_latest_date_recorded" "date", "feeding_latest_date_nonzero" "date", "mortality_latest_date_recorded" "date", "mortality_latest_date_nonzero" "date", "water_quality_rating_numeric_average" double precision, "water_quality_rating_average" "text", "water_quality_latest_date" "date", "water_quality_arrow" "text")
  ...
  select
    ...
    d.input_end_date,
    d.time_period::text,
    d.sampling_start_date,
    d.sampling_end_date,
    d.total_feed_period,
    d.efcr_denominator_period,
    d.efcr,
    d.efcr_latest_date,
    d.efcr_arrow::text,
    d.abw,
    ...
    d.abw_arrow::text,
    d.feeding_rate,
    d.feeding_rate_latest_date,
    d.feeding_rate_arrow::text,
    d.mortality_rate,
    d.mortality_rate_latest_date,
    d.mortality_rate_arrow::text,
    d.biomass_density,
    d.biomass_density_arrow::text,
    d.average_biomass,
    d.average_number_of_fish,
    d.feeding_latest_date_recorded,
    d.feeding_latest_date_nonzero,
    d.mortality_latest_date_recorded,
    d.mortality_latest_date_nonzero,
    d.water_quality_rating_numeric_average,
    d.water_quality_rating_average::text,
    d.water_quality_latest_date,
...
begin
  if p_start_date is not null and p_end_date is not null then
    ...
    into v_start, v_end
    from public.api_time_period_bounds(p_farm_id, v_tp) b;
  end if;
  ...
  cur_metrics as (
    select
      ...
      case when sum(coalesce(biomass_last_sampling,0)) > 0
        then sum(coalesce(feeding_rate,0) * coalesce(biomass_last_sampling,0)) / nullif(sum(coalesce(biomass_last_sampling,0)),0)
        else avg(feeding_rate)
      end as feeding_rate,
      case when sum(coalesce(number_of_fish,0)) > 0
        then sum(coalesce(mortality_rate,0) * coalesce(number_of_fish,0)) / nullif(sum(coalesce(number_of_fish,0)),0)
        else avg(mortality_rate)
      end as mortality_rate,
      avg(biomass_last_sampling) as average_biomass,
      avg(biomass_density) as biomass_density,
      sum(coalesce(feeding_amount,0)) as feed_sum
    from cur
    group by system_id
  ),
  prev_metrics as (
  ...
  cur_biom as (
    select
      s.system_id,
      (select biomass_last_sampling from public.daily_fish_inventory_table where system_id=s.system_id and inventory_date=v_start) as b0,
      (select biomass_last_sampling from public.daily_fish_inventory_table where system_id=s.system_id and inventory_date=v_end) as b1
    from sys s
  ),
  prev_biom as (
    select
      s.system_id,
      (select biomass_last_sampling from public.daily_fish_inventory_table where system_id=s.system_id and inventory_date=v_prev_start) as b0,
      (select biomass_last_sampling from public.daily_fish_inventory_table where system_id=s.system_id and inventory_date=v_prev_end) as b1
    from sys s
  ),
  cur_w as (
    select
      s.system_id,
      coalesce((select sum(total_weight_harvest) from public.fish_harvest where system_id=s.system_id and date > v_start and date <= v_end),0) as harvest_kg,
      coalesce((select sum(total_weight_stocking) from public.fish_stocking where system_id=s.system_id and date > v_start and date <= v_end),0) as stocked_kg,
      coalesce((select sum(total_weight_transfer) from public.fish_transfer where origin_system_id=s.system_id and date > v_start and date <= v_end and public.transfer_impacts_efcr(transfer_type, origin_system_id, target_system_id)),0) as tout_kg,
      coalesce((select sum(total_weight_transfer) from public.fish_transfer where target_system_id=s.system_id and date > v_start and date <= v_end and public.transfer_impacts_efcr(transfer_type, origin_system_id, target_system_id)),0) as tin_kg
    from sys s
  ),
  prev_w as (
    select
      s.system_id,
      coalesce((select sum(total_weight_harvest) from public.fish_harvest where system_id=s.system_id and date > v_prev_start and date <= v_prev_end),0) as harvest_kg,
      coalesce((select sum(total_weight_stocking) from public.fish_stocking where system_id=s.system_id and date > v_prev_start and date <= v_prev_end),0) as stocked_kg,
      coalesce((select sum(total_weight_transfer) from public.fish_transfer where origin_system_id=s.system_id and date > v_prev_start and date <= v_prev_end and public.transfer_impacts_efcr(transfer_type, origin_system_id, target_system_id)),0) as tout_kg,
      coalesce((select sum(total_weight_transfer) from public.fish_transfer where target_system_id=s.system_id and date > v_prev_start and date <= v_prev_end and public.transfer_impacts_efcr(transfer_type, origin_system_id, target_system_id)),0) as tin_kg
    from sys s
  ),
  cur_efcr as (
    select
      ...
      case when cb.b0 is null or cb.b1 is null then null::double precision
           else (cb.b1 - cb.b0 + cw.harvest_kg + cw.tout_kg - cw.tin_kg - cw.stocked_kg)
      end as denom,
      cm.feed_sum
    from sys s
  ...
  prev_efcr as (
    select
      ...
      case when pb.b0 is null or pb.b1 is null then null::double precision
           else (pb.b1 - pb.b0 + pw.harvest_kg + pw.tout_kg - pw.tin_kg - pw.stocked_kg)
      end as denom,
      pm.feed_sum
    from sys s
  ...
  select
    ...
    case when ce.denom is null or ce.denom = 0 then null::double precision
         else (ce.feed_sum / ce.denom)
    end as efcr_period_consolidated,
    cwq.wq_num as water_quality_rating_numeric_average,
    case when cwq.wq_num is null then null else public.water_quality_rating_label(cwq.wq_num) end as water_quality_rating_average,
    (
...
CREATE OR REPLACE FUNCTION "public"."api_efcr_trend"("p_farm_id" "uuid", "p_system_id" bigint DEFAULT NULL::bigint, "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("system_id" bigint, "farm_id" "uuid", "inventory_date" "date", "last_sampling_date" "date", "efcr_period_last_sampling" numeric, "biomass_last_sampling" numeric, "biomass_efcr_multiple" numeric, "system_name" "text")
  ...
  select
    ...
    ev.farm_id,
    ev.inventory_date,
    ev.last_sampling_date,
    ev.efcr_period_last_sampling,
    ev.biomass_last_sampling,
    ev.biomass_efcr_multiple,
    ...
    select
      ...
      case rs.anchor_scope
        when 'water_quality' then (
          select max(a.fact_date)
          from public.analytics_system_day a
          join public.system s on s.id = a.system_id
          where s.farm_id = p_farm_id
            and a.water_quality_rating_numeric is not null
            and (p_anchor_date is null or a.fact_date <= p_anchor_date)
        )
        when 'feeding' then (
          select max(a.fact_date)
          from public.analytics_system_day a
          join public.system s on s.id = a.system_id
          where s.farm_id = p_farm_id
            and a.feeding_amount is not null
            and (p_anchor_date is null or a.fact_date <= p_anchor_date)
        )
        when 'feed_inventory' then (
          select max(f.fact_date)
          from public.analytics_feed_inventory_day f
          where f.farm_id = p_farm_id
            and (p_anchor_date is null or f.fact_date <= p_anchor_date)
        )
        else (
          select max(a.fact_date)
          from public.analytics_system_day a
...
begin
  ...
  -- lethal (everything outside critical band falls here)
  v_distance := least(
...
WITH samp AS (
  ...
  JOIN public.system s
    ON s.id = fsw.system_id
  WHERE fsw.system_id = p_system_id
    AND s.farm_id = p_farm_id
    AND fsw.date >= CURRENT_DATE - p_days
),
intervals AS (
  SELECT
    prev_date AS ps,
    ...
    abw AS abw_end,
    (sample_date - prev_date)::int AS di
  FROM samp
...
feeds AS (
  SELECT
    i.ps,
    i.pe,
    SUM(fr.feeding_amount)::numeric AS fk
  FROM intervals i
  JOIN public.feeding_record fr
    ON fr.system_id = p_system_id
   AND fr.date > i.ps
   AND fr.date <= i.pe
  WHERE fr.feeding_amount > 0
  GROUP BY i.ps, i.pe
)
SELECT
  i.ps,
  i.pe,
  ROUND(COALESCE(f.fk,0),3),
  ROUND(i.wg::numeric,3),
...
begin
  select coalesce(sum(fme.dead_count), 0)::integer
    into v_dead_count
  from public.fish_mortality_events fme
  where fme.system_id = p_system_id
    and fme.event_date = p_event_date
    and (
      (p_batch_id is null and fme.batch_id is null)
      or fme.batch_id = p_batch_id
    );

  delete from public.fish_mortality fm
  where fm.system_id = p_system_id
    and fm.date = p_event_date
    and (
      (p_batch_id is null and fm.batch_id is null)
      or fm.batch_id = p_batch_id
    );
...
CREATE OR REPLACE FUNCTION "public"."trg_sync_mortality_rollup"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  if tg_op in ('UPDATE', 'DELETE') then
    perform public.refresh_mortality_daily_rollup(old.system_id, old.batch_id, old.event_date);
  end if;

  if tg_op in ('INSERT', 'UPDATE') then
    perform public.refresh_mortality_daily_rollup(new.system_id, new.batch_id, new.event_date);
  end if;
...
ALTER FUNCTION "public"."trg_sync_mortality_rollup"() OWNER TO "postgres";
...
CREATE MATERIALIZED VIEW "public"."dashboard" AS
 WITH "anchor" AS (
         ...
         SELECT "si_1"."system_id",
            "ad"."time_period",
            "ad"."input_start_date",
            "ad"."input_end_date",
            ( SELECT "max"("dfit"."last_sampling_date") AS "max"
                   FROM "public"."daily_fish_inventory_table" "dfit"
                  WHERE (("dfit"."system_id" = "si_1"."system_id") AND ("dfit"."last_sampling_date" IS NOT NULL) AND ("dfit"."last_sampling_date" <= "ad"."input_start_date"))) AS "sampling_start_date",
            ( SELECT "max"("dfit"."last_sampling_date") AS "max"
                   FROM "public"."daily_fish_inventory_table" "dfit"
                  WHERE (("dfit"."system_id" = "si_1"."system_id") AND ("dfit"."last_sampling_date" IS NOT NULL) AND ("dfit"."last_sampling_date" <= "ad"."input_end_date"))) AS "sampling_end_date"
           FROM ("system_info" "si_1"
             JOIN "all_dates" "ad" ON (("ad"."farm_id" = "si_1"."farm_id")))
        ), "bounds" AS (
         SELECT "sb"."system_id",
            "sb"."time_period",
            "sb"."input_start_date",
            "sb"."input_end_date",
                CASE
                    WHEN (("sb"."sampling_start_date" IS NULL) OR ("sb"."sampling_end_date" IS NULL)) THEN NULL::"date"
                    WHEN ("sb"."sampling_start_date" = "sb"."sampling_end_date") THEN ( SELECT "max"("dfit"."last_sampling_date") AS "max"
                       FROM "public"."daily_fish_inventory_table" "dfit"
                      WHERE (("dfit"."system_id" = "sb"."system_id") AND ("dfit"."last_sampling_date" IS NOT NULL) AND ("dfit"."last_sampling_date" < "sb"."sampling_start_date")))
                    ELSE "sb"."sampling_start_date"
                END AS "sampling_start_date",
            "sb"."sampling_end_date"
           FROM "sampling_bounds" "sb"
          WHERE (("sb"."sampling_start_date" IS NOT NULL) AND ("sb"."sampling_end_date" IS NOT NULL) AND ("sb"."sampling_end_date" >= "sb"."sampling_start_date"))
        ), "bound_vals" AS (
         SELECT "b"."system_id",
            "b"."time_period",
            "b"."input_start_date",
            "b"."input_end_date",
            "b"."sampling_start_date",
            "b"."sampling_end_date",
            "dfi_s"."biomass_last_sampling" AS "biomass_start",
            "dfi_e"."biomass_last_sampling" AS "biomass_end",
            "dfi_e"."abw_last_sampling" AS "abw",
            "b"."sampling_end_date" AS "abw_latest_date"
           FROM (("bounds" "b"
             LEFT JOIN "public"."daily_fish_inventory_table" "dfi_s" ON ((("dfi_s"."system_id" = "b"."system_id") AND ("dfi_s"."inventory_date" = "b"."sampling_start_date"))))
             LEFT JOIN "public"."daily_fish_inventory_table" "dfi_e" ON ((("dfi_e"."system_id" = "b"."system_id") AND ("dfi_e"."inventory_date" = "b"."sampling_end_date"))))
        ), "inv_window_agg" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            ("sum"(COALESCE("dfit"."feeding_amount", (0)::numeric)))::double precision AS "feed_sum",
            ("sum"(COALESCE("dfit"."number_of_fish_mortality", (0)::numeric)))::double precision AS "mortality_sum",
            "avg"(NULLIF("dfit"."number_of_fish", (0)::double precision)) AS "avg_fish",
            "avg"(NULLIF("dfit"."biomass_last_sampling", (0)::double precision)) AS "avg_biomass",
            ("avg"("dfit"."biomass_density"))::double precision AS "avg_biomass_density"
           FROM ("bound_vals" "bv_1"
             JOIN "public"."daily_fish_inventory_table" "dfit" ON ((("dfit"."system_id" = "bv_1"."system_id") AND ("dfit"."inventory_date" >= "bv_1"."input_start_date") AND ("dfit"."inventory_date" <= "bv_1"."input_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "efcr_mid_daily" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            ("sum"(COALESCE("dfit"."feeding_amount", (0)::numeric)))::double precision AS "feed_sum_sampling",
            ("sum"(COALESCE("dfit"."number_of_fish_mortality", (0)::numeric)))::double precision AS "mortality_sum_sampling"
           FROM ("bound_vals" "bv_1"
             JOIN "public"."daily_fish_inventory_table" "dfit" ON ((("dfit"."system_id" = "bv_1"."system_id") AND ("dfit"."inventory_date" > "bv_1"."sampling_start_date") AND ("dfit"."inventory_date" <= "bv_1"."sampling_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "harvest_sum" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            COALESCE("sum"("fh"."total_weight_harvest"), (0)::double precision) AS "harvested_kg"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."fish_harvest" "fh" ON ((("fh"."system_id" = "bv_1"."system_id") AND ("fh"."date" > "bv_1"."sampling_start_date") AND ("fh"."date" <= "bv_1"."sampling_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "stocking_sum" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            COALESCE("sum"("fs"."total_weight_stocking"), (0)::double precision) AS "stocked_kg"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."fish_stocking" "fs" ON ((("fs"."system_id" = "bv_1"."system_id") AND ("fs"."date" > "bv_1"."sampling_start_date") AND ("fs"."date" <= "bv_1"."sampling_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "transfer_out_sum" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            COALESCE("sum"("ft"."total_weight_transfer"), (0)::double precision) AS "transfer_out_kg"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."fish_transfer" "ft" ON ((("ft"."origin_system_id" = "bv_1"."system_id") AND ("ft"."date" > "bv_1"."sampling_start_date") AND ("ft"."date" <= "bv_1"."sampling_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "transfer_in_sum" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            COALESCE("sum"("ft"."total_weight_transfer"), (0)::double precision) AS "transfer_in_kg"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."fish_transfer" "ft" ON ((("ft"."target_system_id" = "bv_1"."system_id") AND ("ft"."date" > "bv_1"."sampling_start_date") AND ("ft"."date" <= "bv_1"."sampling_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "feed_latest" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            "max"("fr"."date") AS "feeding_latest_date_recorded",
            "max"("fr"."date") FILTER (WHERE (COALESCE("fr"."feeding_amount", (0)::double precision) > (0)::double precision)) AS "feeding_latest_date_nonzero"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."feeding_record" "fr" ON ((("fr"."system_id" = "bv_1"."system_id") AND ("fr"."date" <= "bv_1"."input_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "mort_latest" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            "max"("fm"."date") AS "mortality_latest_date_recorded",
            "max"("fm"."date") FILTER (WHERE (COALESCE("fm"."number_of_fish_mortality", (0)::bigint) > 0)) AS "mortality_latest_date_nonzero"
           FROM ("bound_vals" "bv_1"
             LEFT JOIN "public"."fish_mortality" "fm" ON ((("fm"."system_id" = "bv_1"."system_id") AND ("fm"."date" <= "bv_1"."input_end_date"))))
          GROUP BY "bv_1"."system_id", "bv_1"."time_period", "bv_1"."input_start_date", "bv_1"."input_end_date"
        ), "wq" AS (
         SELECT "bv_1"."system_id",
            "bv_1"."time_period",
            "bv_1"."input_start_date",
            "bv_1"."input_end_date",
            ( SELECT "dwr"."rating_date"
                   FROM "public"."daily_water_quality_rating" "dwr"
                  WHERE (("dwr"."system_id" = "bv_1"."system_id") AND ("dwr"."rating_date" <= "bv_1"."input_end_date"))
                  ORDER BY "dwr"."rating_date" DESC
                 LIMIT 1) AS "water_quality_latest_date",
            ( SELECT ("dwr"."rating_numeric")::double precision AS "rating_numeric"
                   FROM "public"."daily_water_quality_rating" "dwr"
                  WHERE (("dwr"."system_id" = "bv_1"."system_id") AND ("dwr"."rating_date" <= "bv_1"."input_end_date"))
                  ORDER BY "dwr"."rating_date" DESC
                 LIMIT 1) AS "water_quality_rating_numeric_average",
            ( SELECT "dwr"."rating"
                   FROM "public"."daily_water_quality_rating" "dwr"
                  WHERE (("dwr"."system_id" = "bv_1"."system_id") AND ("dwr"."rating_date" <= "bv_1"."input_end_date"))
                  ORDER BY "dwr"."rating_date" DESC
                 LIMIT 1) AS "water_quality_rating_average"
           FROM "bound_vals" "bv_1"
        )
 SELECT "bv"."system_id",
    "si"."farm_id",
    ...
    "bv"."input_end_date",
    "bv"."time_period",
    "bv"."sampling_start_date",
    "bv"."sampling_end_date",
    "wa"."feed_sum" AS "total_feed_period",
    (((((COALESCE("bv"."biomass_end", (0)::double precision) - COALESCE("bv"."biomass_start", (0)::double precision)) + COALESCE("hs"."harvested_kg", (0)::double precision)) + COALESCE("tos"."transfer_out_kg", (0)::double precision)) - COALESCE("tis"."transfer_in_kg", (0)::double precision)) - COALESCE("ss"."stocked_kg", (0)::double precision)) AS "efcr_denominator_period",
        CASE
            WHEN (NULLIF((((((COALESCE("bv"."biomass_end", (0)::double precision) - COALESCE("bv"."biomass_start", (0)::double precision)) + COALESCE("hs"."harvested_kg", (0)::double precision)) + COALESCE("tos"."transfer_out_kg", (0)::double precision)) - COALESCE("tis"."transfer_in_kg", (0)::double precision)) - COALESCE("ss"."stocked_kg", (0)::double precision)), (0)::double precision) IS NULL) THEN NULL::double precision
            ELSE ("emd"."feed_sum_sampling" / NULLIF((((((COALESCE("bv"."biomass_end", (0)::double precision) - COALESCE("bv"."biomass_start", (0)::double precision)) + COALESCE("hs"."harvested_kg", (0)::double precision)) + COALESCE("tos"."transfer_out_kg", (0)::double precision)) - COALESCE("tis"."transfer_in_kg", (0)::double precision)) - COALESCE("ss"."stocked_kg", (0)::double precision)), (0)::double precision))
        END AS "efcr",
    "bv"."sampling_end_date" AS "efcr_latest_date",
    'straight'::"public"."arrows" AS "efcr_arrow",
    "bv"."abw",
    "bv"."abw_latest_date",
    'straight'::"public"."arrows" AS "abw_arrow",
        CASE
            WHEN (("wa"."avg_biomass" IS NULL) OR ("wa"."avg_biomass" <= (0)::double precision)) THEN NULL::double precision
            ELSE ("wa"."feed_sum" / "wa"."avg_biomass")
        END AS "feeding_rate",
    "fl"."feeding_latest_date_recorded" AS "feeding_rate_latest_date",
    'straight'::"public"."arrows" AS "feeding_rate_arrow",
        CASE
            WHEN (("wa"."avg_fish" IS NULL) OR ("wa"."avg_fish" <= (0)::double precision)) THEN NULL::double precision
            ELSE ("wa"."mortality_sum" / "wa"."avg_fish")
        END AS "mortality_rate",
    "ml"."mortality_latest_date_recorded" AS "mortality_rate_latest_date",
    'straight'::"public"."arrows" AS "mortality_rate_arrow",
        CASE
            WHEN (("si"."volume" IS NULL) OR ("si"."volume" <= (0)::double precision)) THEN NULL::double precision
            ELSE "wa"."avg_biomass_density"
        END AS "biomass_density",
    'straight'::"public"."arrows" AS "biomass_density_arrow",
    "wa"."avg_biomass" AS "average_biomass",
    "wa"."avg_fish" AS "average_number_of_fish",
    "fl"."feeding_latest_date_recorded",
    "fl"."feeding_latest_date_nonzero",
    "ml"."mortality_latest_date_recorded",
    "ml"."mortality_latest_date_nonzero",
    "wq"."water_quality_rating_numeric_average",
    "wq"."water_quality_rating_average",
    "wq"."water_quality_latest_date",
    'straight'::"public"."arrows" AS "water_quality_arrow"
   FROM (((((((((("bound_vals" "bv"
     JOIN "system_info" "si" ON (("si"."system_id" = "bv"."system_id")))
     LEFT JOIN "inv_window_agg" "wa" ON ((("wa"."system_id" = "bv"."system_id") AND ("wa"."time_period" = "bv"."time_period") AND ("wa"."input_start_date" = "bv"."input_start_date") AND ("wa"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "efcr_mid_daily" "emd" ON ((("emd"."system_id" = "bv"."system_id") AND ("emd"."time_period" = "bv"."time_period") AND ("emd"."input_start_date" = "bv"."input_start_date") AND ("emd"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "harvest_sum" "hs" ON ((("hs"."system_id" = "bv"."system_id") AND ("hs"."time_period" = "bv"."time_period") AND ("hs"."input_start_date" = "bv"."input_start_date") AND ("hs"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "stocking_sum" "ss" ON ((("ss"."system_id" = "bv"."system_id") AND ("ss"."time_period" = "bv"."time_period") AND ("ss"."input_start_date" = "bv"."input_start_date") AND ("ss"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "transfer_out_sum" "tos" ON ((("tos"."system_id" = "bv"."system_id") AND ("tos"."time_period" = "bv"."time_period") AND ("tos"."input_start_date" = "bv"."input_start_date") AND ("tos"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "transfer_in_sum" "tis" ON ((("tis"."system_id" = "bv"."system_id") AND ("tis"."time_period" = "bv"."time_period") AND ("tis"."input_start_date" = "bv"."input_start_date") AND ("tis"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "feed_latest" "fl" ON ((("fl"."system_id" = "bv"."system_id") AND ("fl"."time_period" = "bv"."time_period") AND ("fl"."input_start_date" = "bv"."input_start_date") AND ("fl"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "mort_latest" "ml" ON ((("ml"."system_id" = "bv"."system_id") AND ("ml"."time_period" = "bv"."time_period") AND ("ml"."input_start_date" = "bv"."input_start_date") AND ("ml"."input_end_date" = "bv"."input_end_date"))))
     LEFT JOIN "wq" ON ((("wq"."system_id" = "bv"."system_id") AND ("wq"."time_period" = "bv"."time_period") AND ("wq"."input_start_date" = "bv"."input_start_date") AND ("wq"."input_end_date" = "bv"."input_end_date"))))
  WITH NO DATA;
...
CREATE INDEX "idx_dashboard_system_period_end_desc" ON "public"."dashboard" USING "btree" ("system_id", "time_period", "input_end_date" DESC);
...
GRANT ALL ON FUNCTION "public"."trg_sync_mortality_rollup"() TO "service_role";
...
GRANT SELECT ON TABLE "public"."feed_incoming" TO "authenticated";
...
GRANT SELECT ON TABLE "public"."dashboard_time_period" TO "metabase_reader";



GRANT ALL ON TABLE "public"."dashboard" TO "service_role";
GRANT SELECT ON TABLE "public"."dashboard" TO "metabase_reader";
...
GRANT SELECT ON TABLE "public"."efcr_period_last_sampling_view" TO "metabase_reader";

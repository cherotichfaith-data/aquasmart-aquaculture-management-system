...
export default function TransferForm({ onSubmit }: { onSubmit: () => void }) {
  const [formData, setFormData] = useState({
    date: new Date().toISOString().split("T")[0],
    originCage: "Cage-1",
    destinationCage: "Cage-2",
    fishCount: "",
    totalWeight: "",
  ...
  return (
    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1">Origin System</label>
          <select
            name="originCage"
            value={formData.originCage}
            onChange={handleChange}
            className="w-full px-3 py-2 border border-input rounded-lg bg-background"
          >
        ...
        <div>
          ...
          <select
            name="destinationCage"
            value={formData.destinationCage}
            onChange={handleChange}
            className="w-full px-3 py-2 border border-input rounded-lg bg-background"
          >
            <option>Cage-1</option>
            ...
            <option>Cage-3</option>
            <option>Pond-1</option>
            <option>TAFIRI</option>
            <option>Kimbwela</option>
          </select>
        </div>
      </div>

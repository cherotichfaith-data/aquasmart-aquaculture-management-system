...
type SystemListItem = Database["public"]["Functions"]["api_system_options_rpc"]["Returns"][number]
...
export async function getSystemOptions(params?: {
  ...
  const res = await rpcOrEmpty<SystemListItem>(
    "getSystemOptions",
    (supabase) =>
      queryOptionsRpc(supabase, "api_system_options_rpc", {
        p_farm_id: farmId,

...
export function useRecordMortality() {
  ...
  return useMutation({
    mutationFn: async (payload: MortalityEventInsert) => {
      ...
      const user = await getSessionUser(supabase, "insertData:fish_mortality_events:getSession")
      if (!user) {
    ...
    onMutate: (payload) => {
      addOptimisticActivity(queryClient, { tableName: "fish_mortality_events" })
      const previous = addOptimisticRecentEntry(queryClient, {
        ...
        entry: {
          ...
          batch_id: payload.batch_id ?? null,
          number_of_fish_mortality: payload.dead_count,

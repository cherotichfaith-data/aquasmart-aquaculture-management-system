...
import { normalizeFeedingResponse } from "@/app/feed/feed-analytics"
...
function buildDeathsBySystemAndDate(rows: MortalityEventRow[]) {
  ...
  rows.forEach((row) => {
    map.set(`${row.system_id}:${row.event_date}`, (map.get(`${row.system_id}:${row.event_date}`) ?? 0) + row.dead_count)
  })
  return map
}
...
function buildRepeatedLowDoBySystem(rows: MortalityEventRow[], measurements: WaterQualityMeasurementRow[]) {
  ...
  rows.forEach((row) => {
    const sameDay = doBySystemDate.get(`${row.system_id}:${row.event_date}`) ?? []
    const previousDay = doBySystemDate.get(`${row.system_id}:${addDays(row.event_date, -1)}`) ?? []
...
export function buildMortalityRiskRows(params: {
  ...
  const rows: MortalityRiskRow[] = systemIds.map((systemId) => {
    ...
    const recentEvents = systemEvents.filter((row) => row.event_date >= sevenDayStart && row.event_date <= params.todayDate)
    const deathsToday = recentEvents
      .filter((row) => row.event_date === params.todayDate)
      .reduce((sum, row) => sum + row.dead_count, 0)
    const deaths7d = recentEvents.reduce((sum, row) => sum + row.dead_count, 0)
    const mortalityDays7d = new Set(recentEvents.map((row) => row.event_date)).size
...
export function buildDeathsTrend(events: MortalityEventRow[]) {
  ...
  events.forEach((row) => {
    byDate.set(row.event_date, (byDate.get(row.event_date) ?? 0) + row.dead_count)
  })
